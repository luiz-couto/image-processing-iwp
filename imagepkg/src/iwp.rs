    ...
    T: Clone + Send + Sync + 'static,
{
    ...
    // let mut img_2 = base_img.clone();
    // let aux_s = Arc::new(aux_structure.clone());

    // let mut handles = vec![];
    // let count = 0;

    // let p_img = ParallelImg::new(base_img, num_threads);
    ...
    //                     if propagation_condition(&img_sec, curr_pixel_abs, ngb_pixel_abs, &aux_s) {
    //                         let new_value =
    //                             update_func(&img_sec, curr_pixel_abs, ngb_pixel_abs, &aux_s);
